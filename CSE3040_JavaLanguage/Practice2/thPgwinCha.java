...
public class thPgwinCha extends Frame implements ActionListener { 
	private Canvas cvas; 
	public thPgwinCha() { 
		cvas = new Canvas();         
	...
	public void actionPerformed(ActionEvent evt) {  
		if (evt.getActionCommand().equals("Penguin")) 
			new thIdong(cvas, "src\\pgwin.gif", 0).start();    
		else if (evt.getActionCommand().equals("Car")) 
			new thIdong(cvas, "src\\CHA.GIF", 1).start(); 
		else if (evt.getActionCommand().equals("Exit")) 
			System.exit(0);                
	}
	
	public static void main(String[] args) { 
		Frame f = new thPgwinCha(); 
		f.setSize(500, 300); 
