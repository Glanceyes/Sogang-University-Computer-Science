...
syscall_init (void) 
{
  ...
  lock_init(&page_alloc_lock);
  intr_register_int (0x30, 3, INTR_ON, syscall_handler, "syscall");
}

struct page_entry *check_valid_address(const void *addr)
{
  // if (0x08048000 > (uint32_t)addr || (uint32_t)addr >= 0xc0000000){
  //   syscall_exit(-1);
  // }
  ...
  return page_lookup(addr);
}

static void get_syscall_args(int syscall_num, void *args[], void *esp){
...
static void check_valid_buffer(void *buffer, unsigned size){
  for (int i = 0; i < size; i++){
    struct page_entry* page = check_valid_address((uint8_t *)buffer + i);
    if (page == NULL){
      syscall_exit(-1);
    }
...
syscall_handler (struct intr_frame *f UNUSED) 
{
  ...
  switch(syscall_num) {
    ...
    case SYS_READ:
      // check_valid_buffer((void *)*(uint32_t *)args[1], (unsigned)*(uint32_t *)args[2]);
...
int syscall_read(int fd, void *buffer, unsigned length) {
  ...
  page_unpin_buffer(buffer, length);
