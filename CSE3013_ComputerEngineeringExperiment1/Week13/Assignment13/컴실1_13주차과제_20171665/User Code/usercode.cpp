...
static void drawBuffered(){
	...
	if(dfs_check == 1){
		...
		DrawSolidBox_I(4 * MazeWidth -3, 4 * MazeHeight - 3, 4 * MazeWidth, 4 * MazeHeight, 1, RGB(255,255,0), RGB(255,255,0));
		// 갔던 곳 다 출력
		for(int i = 0;i < MazeHeight;i++){
			for(int j = 0;j < MazeWidth;j++){
				if(DS[i][j].chk==1){
					DrawSolidBox_I(j*4+2,i*4+2,j*4+3,i*4+3,1,RGB(102,204,255),RGB(102,204,255));
					if(i-1 >= 0 && DS[i-1][j].chk == 1 && DS[i-1][j].under == 1)
						DrawSolidBox_I(j*4+2,(i-1)*4+3,j*4+3,i*4+2,1,RGB(102,204,255),RGB(102,204,255));
					if(j-1 >= 0 && DS[i][j-1].chk == 1 && DS[i][j-1].right == 1)
						DrawSolidBox_I((j-1)*4+3,i*4+2,j*4+2,i*4+3,1,RGB(102,204,255),RGB(102,204,255));
				}
			}
		}
		// 정답 출력
...
void DFS(){
	...
	while(stack_cnt != 0){ // stack이 빌 때까지
		...
		if(temp.x + 1 < MazeHeight && DS[temp.x][temp.y].under == 1 && DS[temp.x+1][temp.y].chk == 0 && chk == 0){ // 아래쪽으로 갈 수 있으면
			chk = 1;
		...
		if(temp.y+1 < MazeWidth && DS[temp.x][temp.y].right == 1 && DS[temp.x][temp.y+1].chk == 0 && chk == 0){ // 오른쪽으로 갈 수 있으면
			chk=1;
		...
		if(temp.y > 0 && DS[temp.x][temp.y-1].right == 1 && DS[temp.x][temp.y-1].chk == 0 && chk == 0){ // 왼쪽으로 갈 수 있으면
			chk=1;
			DS[temp.x][temp.y - 1].chk = 1; // 방문한 것으로 처리
			temp2.x = temp.x;
			temp2.y = temp.y - 1;
		...
		if(temp.x>0&&DS[temp.x-1][temp.y].under==1&&DS[temp.x-1][temp.y].chk==0&&chk==0){ // 위쪽으로 갈 수 있으면
			chk=1;
			DS[temp.x - 1][temp.y].chk = 1; // 방문한 것으로 처리
			temp2.x = temp.x-1;
			temp2.y = temp.y;
