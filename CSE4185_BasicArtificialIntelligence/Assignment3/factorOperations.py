...
def joinFactors(factors: ValuesView[Factor]):
    ...
    joinedConditionedVariables.difference_update(joinedUnconditionedVariables)
    
    # Since all factors have the same variableDomainsDict, just use the first factor.
    joinedVariableDomainsDict = list(factors)[0].variableDomainsDict()
    joinedFactor = Factor(joinedUnconditionedVariables, joinedConditionedVariables, joinedVariableDomainsDict)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        for unconditionedVariable in factor.unconditionedVariables():
            if unconditionedVariable != eliminationVariable:
                unconditionedVariables.add(unconditionedVariable)
        for conditionedVariable in factor.conditionedVariables():
            conditionedVariables.add(conditionedVariable)
        ...
        for assignmentDict in eliminatedFactor.getAllPossibleAssignmentDicts():
            ...
            for value in variableDomainsDict[eliminationVariable]:
                ...
                tempAssignmentDict[eliminationVariable] = value
                probability += factor.getProbability(tempAssignmentDict)
