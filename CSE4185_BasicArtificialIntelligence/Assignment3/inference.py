...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # for pac_x, pac_y in variableDomainsDict[PAC]:
    #     for ghost_x, ghost_y in variableDomainsDict[GHOST0]:
    #         distance = abs(manhattanDistance((pac_x, pac_y), (ghost_x, ghost_y)))
    #         for noise in range(MAX_NOISE + 1):
    #             if distance + noise not in variableDomainsDict[OBS0]:
    #                 variableDomainsDict[OBS0].append(distance + noise)
    #             if distance - noise not in variableDomainsDict[OBS0] and distance - noise >= 0:
    #                 variableDomainsDict[OBS0].append(distance - noise)
        
    #     for ghost_x, ghost_y in variableDomainsDict[GHOST1]:
    #         distance = abs(manhattanDistance((pac_x, pac_y), (ghost_x, ghost_y)))
    #         for noise in range(MAX_NOISE + 1):
    #             if distance + noise not in variableDomainsDict[OBS1]:
    #                 variableDomainsDict[OBS1].append(distance + noise)
    #             if distance - noise not in variableDomainsDict[OBS1] and distance - noise >= 0:
    #                 variableDomainsDict[OBS1].append(distance - noise)
    ...
    #         temp_distance = 0 if (distance - MAX_NOISE) < 0 else (distance - MAX_NOISE)
    #         if temp_distance not in variableDomainsDict[OBS0]:
    #             variableDomainsDict[OBS0].append(temp_distance)
    #     for ghost_x, ghost_y in variableDomainsDict[GHOST1]:
    #         distance = abs(manhattanDistance((pac_x, pac_y), (ghost_x, ghost_y)))
    #         if distance + MAX_NOISE not in variableDomainsDict[OBS1]:
    #             variableDomainsDict[OBS1].append(distance + MAX_NOISE)
    #         temp_distance = 0 if (distance - MAX_NOISE) < 0 else (distance - MAX_NOISE)
    #         if temp_distance not in variableDomainsDict[OBS1]:
    #             variableDomainsDict[OBS1].append(temp_distance)

    # range: 0 ~ (X_RANGE - 1) + (Y_RANGE - 1) + MAX_RANGE = 0 ~ (X_RANGE + Y_RANGE + MAX_RANGE - 2)
    # To use range function, we need to add 1 to the range.
    # Therefore, the range is (X_RANGE + Y_RANGE + MAX_RANGE - 1).

    maximum_observation_value = X_RANGE + Y_RANGE + MAX_NOISE - 1
    variableDomainsDict[OBS0] = [i for i in range(maximum_observation_value)]
    variableDomainsDict[OBS1] = [i for i in range(maximum_observation_value)]
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # print("factors with evidence")
        # for factor in factors:
        #     print(factor)
        for variableToEliminate in eliminationOrder:
            factors, joinedFactor = joinFactorsByVariable(factors, variableToEliminate)
            # print("joined")
            # print(joinedFactor.__str__())
            if len(joinedFactor.unconditionedVariables()) > 1:
                eliminatedFactor = eliminate(joinedFactor, variableToEliminate)
                # print("eliminated")
                # print(eliminatedFactor.__str__())
